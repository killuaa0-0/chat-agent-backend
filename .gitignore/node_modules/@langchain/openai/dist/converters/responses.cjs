...
let openai_lib_responses_ResponseInputItems_js = require("openai/lib/responses/ResponseInputItems.js");
...
const convertResponsesDeltaToChatGenerationChunk = (event) => {
	...
	} else if (event.type === "response.output_item.done" && event.item.type === "reasoning" && event.item.encrypted_content) additional_kwargs.reasoning = { encrypted_content: event.item.encrypted_content };
...
const convertStandardContentMessageToResponsesInput = (message) => {
	...
	function* iterateItems() {
		...
		const convertReasoningBlock = (block) => {
			...
			const encryptedContent = block.encrypted_content;
			return {
				type: "reasoning",
...
const convertMessagesToResponsesInput = ({ messages, zdrEnabled, model }) => {
 ...
 /**
		...
		if (role === "assistant") {
			if (responseMetadata?.output != null && Array.isArray(responseMetadata.output) && responseMetadata.output.length > 0 && responseMetadata.output.every((item) => typeof item === "object" && item != null && "type" in item && typeof item.type === "string")) {
				const output = responseMetadata.output;
				return zdrEnabled ? (0, openai_lib_responses_ResponseInputItems_js.toResponseInputItems)(output) : output;
			}
			const input = [];
		...
		if (role === "user" || role === "system" || role === "developer") {
			...
			const content = lcMsg.content.flatMap((item) => {
				...
				if (item.type === "configuration_update") messages.push({
